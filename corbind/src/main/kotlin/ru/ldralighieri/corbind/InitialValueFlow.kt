...
/** A cold [Flow] whose first emission represents the state at collection time. */
class InitialValueFlow<T>(private val flow: Flow<T>) : Flow<T> by flow {
    /** Returns a flow that skips the initial state of each collection. */
    fun dropInitialValue(): Flow<T> = drop(1)

    /** Starts collecting in [scope] and suspends until the first state is emitted. */
