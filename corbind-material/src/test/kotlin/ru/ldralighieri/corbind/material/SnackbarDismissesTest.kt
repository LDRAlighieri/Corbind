...
class SnackbarDismissesTest {

    @Test
    fun `dismiss callback emits reason and cancellation removes callback`() = runBlocking {
        ...
        val dismisses = snackbar.dismisses()
        assertEquals(0, snackbar.testCallbacks().size)

        dismisses.assertEventAndCleanup(
            expected = Snackbar.Callback.DISMISS_EVENT_MANUAL,
            isRegistered = { snackbar.testCallbacks().size == 1 },
            fire = {
                snackbar.testCallbacks().single().onDismissed(
                    snackbar,
