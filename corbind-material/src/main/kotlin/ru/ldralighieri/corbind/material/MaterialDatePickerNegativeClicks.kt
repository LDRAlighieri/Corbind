...
/**
 * Perform an action on [MaterialDatePicker] negative button clicks.
 *
...
/**
 * Perform an action on [MaterialDatePicker] negative button clicks, in a new [CoroutineScope].
 *
 * @param capacity Capacity of the channel's buffer (no buffer by default). With suspending overflow,
 * events wait for delivery without blocking the Android callback thread.
 * @param action An action to perform
...
/**
 * Create a channel that emits [MaterialDatePicker] negative button clicks.
 *
 * Example:
 *
 * ```
...
/**
 * Create a flow that emits [MaterialDatePicker] negative button clicks.
 *
 * Example:
 *
 * ```
 ...
 */
@CheckResult
fun <S> MaterialDatePicker<S>.negativeClicks(): Flow<Unit> = corbindCallbackFlow {
